...
void RemoteSwitch::sendTelegram2(unsigned long data, unsigned int periodusec, unsigned short repeats) {
	sendTelegram2(data,periodusec,repeats,_pin);	
}

void RemoteSwitch::sendTelegram2(unsigned long data, unsigned int periodusec, unsigned short repeats, unsigned short pin) {
...
// Format HomeEasy EU
void RemoteSwitch::sendTelegram3(unsigned long data1, unsigned long data2, unsigned int periodusec, unsigned short repeats) {
	sendTelegram3(data1,data2,periodusec,repeats,_pin);	
}

void RemoteSwitch::sendTelegram3(unsigned long data1, unsigned long data2, unsigned int periodusec, unsigned short repeats, unsigned short pin) {
	repeats = 1 << (repeats & B111);
	//Reverse the bits
...
void KaKuSwitch2::sendSignal(unsigned long address, boolean group, unsigned short device, boolean on) {
	...
	data |= (on & B1) << 4;
	data |= (device & 0xf);
	sendTelegram2(data, _periodusec, _repeats, _pin);
}
...
void NoName1Switch::sendSignal(unsigned short button) {		
	switch (button) {
		case 0:
			sendTelegram(getTelegram(12), _pin);	// switch both channels off
			break;
		case 1:
			sendTelegram(getTelegram(12), _pin);	// switch both channels off
			delay (20);
		...
		case 2:
			sendTelegram(getTelegram(12), _pin);	// switch both channels off
			delay (20);
			sendTelegram(getTelegram(2), _pin);	// switch channel two on
			break;
		case 3:
			sendTelegram(getTelegram(3), _pin);	// switch both channels on
			break;
	}
}
...
void HomeEasyEUSwitch::sendSignal(unsigned long address, boolean group, unsigned short device, boolean on) {
	...
	sendTelegram3(data1, data2, _periodusec, _repeats, _pin);
}
